...
try:
    env = gym.make_vec("Pendulum-v1", num_envs=4, vectorization_mode="sync", render_mode="rgb_array")
...
# Evaluación manual
num_episodios = 20
episodios_finalizados = 0
recompensas_por_episodio = []
recompensa_actual = 0.0
timestep = 0
states, infos = env.reset()

while episodios_finalizados < num_episodios:
    with torch.no_grad():
        actions = agent.act(states=states, timestep=timestep, timesteps=250)[0]

    next_states, rewards, terminated, truncated, infos = env.step(actions)
    env.render()

    recompensa_actual += rewards[0].item()

    if terminated[0] or truncated[0]:
        recompensas_por_episodio.append(recompensa_actual)
        recompensa_actual = 0.0
        episodios_finalizados += 1
        states, infos = env.reset()
    else:
        states = next_states
...
media_recompensas = sum(recompensas_por_episodio) / len(recompensas_por_episodio)
print(f"Recompensas por episodio: {recompensas_por_episodio}")
print(f"Recompensa media en {num_episodios} episodios: {media_recompensas:.2f}")
